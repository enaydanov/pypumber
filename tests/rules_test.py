...
class TestGetFuncArgs(unittest.TestCase):
    def test_000_noargs(self):
        def tmp():
            return 1
        self.assertEqual(rules.get_func_args(tmp), ((), None, None, 0))

    def test_001_one_arg(self):
        def tmp(a):
            return a
        self.assertEqual(rules.get_func_args(tmp), (('a',), None, None, 1))

    def test_002_two_args(self):
        def tmp(a, b):
            c = a + b
            return c
        self.assertEqual(rules.get_func_args(tmp), (('a', 'b'), None, None, 2))

    def test_003_only_arg(self):
        def tmp(*arg):
            c = len(arg)
            return c
        self.assertEqual(rules.get_func_args(tmp), ((), 'arg', None, 1))

    def test_004_one_arg_with_arg(self):
        def tmp(a, *arg):
            c = a + len(arg)
            return c
        self.assertEqual(rules.get_func_args(tmp), (('a',), 'arg', None, 2))

    def test_005_two_args_with_arg(self):
        def tmp(a, b, *arg):
            c = a + b + len(arg)
            return c
        self.assertEqual(rules.get_func_args(tmp), (('a', 'b'), 'arg', None, 3))

    def test_006_only_kw(self):
        def tmp(**kw):
            c = len(kw.items())
            return c
        self.assertEqual(rules.get_func_args(tmp), ((), None, 'kw', 1))

    def test_007_one_arg_with_kw(self):
        def tmp(a, **kw):
            c = a + len(kw.items())
            return c
        self.assertEqual(rules.get_func_args(tmp), (('a',), None, 'kw', 2))

    def test_008_two_args_with_kw(self):
        def tmp(a, b, **kw):
            c = a + b + len(kw.items())
            return c
        self.assertEqual(rules.get_func_args(tmp), (('a', 'b'), None, 'kw', 3))

    def test_009_only_arg_and_kw(self):
        def tmp(*arg, **kw):
            c = len(arg) + len(kw.items())
            return c
        self.assertEqual(rules.get_func_args(tmp), ((), 'arg', 'kw', 2))

    def test_010_one_arg_arg_and_kw(self):
        def tmp(a, *arg, **kw):
            c = a + len(arg) + len(kw.items())
            return c
        self.assertEqual(rules.get_func_args(tmp), (('a',), 'arg', 'kw', 3))

    def test_011_two_args_arg_and_kw(self):
        def tmp(a, b, *arg, **kw):
            c = a + b + len(arg) + len(kw.items())
            return c
        self.assertEqual(rules.get_func_args(tmp), (('a', 'b'), 'arg', 'kw', 4))
...
class TestRules(unittest.TestCase):
    ...
    def test_008_given_rule_match_not_found(self):
        self.assertRaises(rules.MatchNotFound, self.r.given, "10 - 5")
    ...
    def test_010_given_rule_ambiguous(self):
        ...
        self.assertRaises(rules.AmbiguousString, self.r.given, "10 - 5")

    def test_020_load_from_file(self):
        self.r.load_from_file('example_import.py')
